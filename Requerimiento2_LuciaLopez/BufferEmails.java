...
public class BufferEmails {
	...
	public synchronized void addEmail(Emails email) {
		// Indicamos con un while que si el buffer est� lleno, tendr� que esperar hasta
		// que se saquen elementos.
		...
		// Para simular que los emails se irán poniendo en el buffer cada 0,5 segundos,
		// creamos un Thread.sleep
		// Esto también podría realizarse en el Productor de Emails, en lugar de en el
		// propio Buffer.
		try {
			Thread.sleep(500);
		} catch (InterruptedException e) {
		...
		// Requerimiento 3. Para evitar que se produzcan emails que se envíen a
		// pikachu@gmail.com, creamos un if,
		// igualando el destinatario con ese email y descartandolo. Si quisiesemos que
		// el Email no llegase a entrar en el Buffer
		...
		if (email.getDestinatario().equals("pikachu@gmail.com")) {
			...
			buffer.offer(email);
		// Con notify, notificamos a todos los hilos que esten en estado wait, a que se
		// despierten
		notify();
	}
	...
	public synchronized Emails getEmail() {
		Emails e = null;
		// Indicamos con un while que si el buffer esta vacio, tendra que esperar hasta
		// que se introduzcan elementos.
		while (buffer.size() == 0) {
			try {
				wait();
